...
export async function POST(req: Request) {
  try {
    ...
    if (!['APPROVED', 'REJECTED'].includes(status)) {
      return NextResponse.json({ ok: false, error: 'Invalid status. Must be APPROVED or REJECTED.' }, { status: 400 });
    }

    const updates = status === 'APPROVED'
      ? {
          subscription_submission_status: 'APPROVED',
          available_on_celite_subscription: true,
        }
      : {
          subscription_submission_status: 'REJECTED',
