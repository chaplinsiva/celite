  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      const { data: category } = await supabase
        .from('categories')
        .select('id')
        .eq('slug', soundEffectsCategorySlug)
        .maybeSingle();
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {hasActiveFilters && (
                    <button
                      ...
                      className="text-xs sm:text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1 p-1"
          ...
          <div className="flex-1 min-w-0">
            {filteredTemplates.length > 0 ? (
              ...
              <div className="text-center py-20 bg-white rounded-2xl border border-zinc-200 border-dashed">
                ...
                <h3 className="text-xl font-bold text-zinc-900 mb-2">No sound effects found</h3>
                <p className="text-zinc-500 mb-6">We couldn't find any sound effects matching your search.</p>
                {hasActiveFilters && (
                  <button
                    onClick={clearFilters}
